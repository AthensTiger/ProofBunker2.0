...
function QAItem({ question, ticketId }: { question: TicketQuestion; ticketId: number }) {
  ...
  const handleSubmit = () => {
    ...
    respondMutation.mutate(
      { id: ticketId, qid: question.id, response: responseText.trim(), files: attachments },
      {
        onSuccess: () => { addToast('success', 'Response sent'); setResponseText(''); setAttachments([]); },
    ...
    <div className="border border-amber-200 bg-amber-50 rounded-lg p-3 space-y-2">
      ...
      {question.response ? (
        <div className="border-t border-amber-200 pt-2 space-y-2">
          ...
          {question.response_attachments && question.response_attachments.length > 0 && (
            <div className="flex flex-wrap gap-2 pt-1">
              {question.response_attachments.map((att) => (
                <a
                  key={att.id}
                  href={att.cdn_url}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="group relative block w-16 h-16 rounded-lg overflow-hidden border border-gray-200 hover:border-amber-400 transition-colors"
                  title={att.filename}
                >
        ...
        <div className="border-t border-amber-200 pt-2 space-y-2">
          ...
          <div className="flex items-center justify-between">
            {attachments.length < 5 ? (
              ...
              <span className="text-xs text-gray-400">5/5 images attached</span>
            )}
            <button
